...
export function HandwritingSvg({
  ...
  const vwNum = parseFloat(parts[2] || "") || width;
  const vhNum = parseFloat(parts[3] || "") || height;
  const proportionalWidth = Math.max(width, Math.ceil((height * vwNum) / (vhNum || 1)));
  ...
  return (
    <svg
      ...
      height={height}
      viewBox={svgViewBox}
      className={cn("text-rose-500 max-w-full h-auto", className)}
      aria-hidden={true}
    >
      <title>Handwriting SVG</title>
      <motion.path
