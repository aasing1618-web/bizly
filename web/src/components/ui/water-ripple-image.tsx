...
export default function WaterRippleImage({
  ...
  const uniformsRef = useRef<Map<string, WebGLUniformLocation | null>>(new Map());
  ...
  const getUniform = (name: string): WebGLUniformLocation | null => {
    return uniformsRef.current.get(name) ?? null;
  };

  const updateUniforms = (gl: WebGLRenderingContext) => {
    gl.uniform1f(getUniform('u_blueish'), params.blueish);
    gl.uniform1f(getUniform('u_scale'), params.scale);
    gl.uniform1f(getUniform('u_illumination'), params.illumination);
    gl.uniform1f(getUniform('u_surface_distortion'), params.surfaceDistortion);
    gl.uniform1f(getUniform('u_water_distortion'), params.waterDistortion);
  };
  ...
  const setTextureFromImage = (gl: WebGLRenderingContext, image: HTMLImageElement) => {
    ...
    gl.uniform1i(getUniform('u_image_texture'), 0);

    const imgRatio = image.naturalWidth / image.naturalHeight;
    const canvas = canvasRef.current!;
    gl.uniform1f(getUniform('u_ratio'), canvas.width / canvas.height);
    gl.uniform1f(getUniform('u_img_ratio'), imgRatio);
  };
  ...
  const resize = () => {
    ...
    const w = Math.max(10, Math.floor((rect.width || 400) * dpr));
    const h = Math.max(10, Math.floor((rect.height || 300) * dpr));
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < uniformCount; i++) {
      ...
      if (!info) continue;
      uniformsRef.current.set(info.name, gl.getUniformLocation(program, info.name));
    }
    ...
    const render = () => {
      const timeLoc = getUniform('u_time');
      if (timeLoc) {
